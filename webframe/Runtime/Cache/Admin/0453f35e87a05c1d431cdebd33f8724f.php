...
<html>
	...
	<body>
		<div id="page-content">
			<div id="main-container container-fluid" style="margin-left:40px;">
				<div id="headshow" >
					<button type="button" class="btn btn-success btn-sm" onclick="add();"><span class="glyphicon glyphicon-plus"></span>职称选取</button>
	...
	<script type="text/javascript">	
		$('#table').bootstrapTable({
					...
					method: 'get',
					url: "/yisheng/webframe/index.php/Admin/LecDuty/ajaxIndex",
					//cache: false,
		...
		function handle(value, row, index) {
				...
				var btnIsEnable='';
				if(row.isenable=="0"){
					btnIsEnable='&nbsp;&nbsp;<a class="remove ml10 btn btn-xs btn-outline btn-default" href="javascript:void(0)" onclick="setNewsCate('+row.lec_duty_id+',1)" title="设为禁用"><i class="glyphicon glyphicon-remove-circle"></i></a>';
				}
				else{
					btnIsEnable='&nbsp;&nbsp;<a class="remove ml10 btn btn-xs btn-outline btn-default" href="javascript:void(0)" onclick="setNewsCate('+row.lec_duty_id+',0)" title="设为启用"><i class="glyphicon glyphicon-off"></i></a>';
				}
				return [
						'<a class="remove ml10 btn btn-xs btn-outline btn-default" href="javascript:void(0)" onclick="delmore('+row.lec_duty_id+')" title="删除">',
			...
			function queryParams(params) {
				...
				if (typeof (params.sort) == "undefined") {
					params.sort = 'lec_duty_ano.lec_duty_id'; //默认排序字段
					params.order = 'desc';
				}
		...
		*/
		function add(){
			var index = layer.open({
						type: 2,
						skin: 'demo-class',
						title: ['职称选择', 'font-size:14px;color:black;'],
						...
						content: ['/yisheng/webframe/index.php/Admin/LecDuty/add', 'no'],
						btn: ['确定', '取消']
						, yes: function (index) {
						...
						if(cate_id==""){
							...
							return;
								...
								$.ajax({
									type: 'post',
									url: '/yisheng/webframe/index.php/Admin/LecDuty/insert',
									data: obj.serialize(),
									cache: false,
		...
		*/
		function setNewsCate(id,flag){
			$.ajax({
				...
				url:"/yisheng/webframe/index.php/Admin/LecDuty/setLecDutyIsEnable",
				success:function(data){
					if (data.status) {
											layer.msg("设置成功", {
											icon: 1,
			...
			layer.confirm('确定要删除吗？', {
					...
					if (!lec_duty_id) {
						...
						$.each(obj, function (n, value) {
							ids += value.lec_duty_id + ',';
						});
					} else {
						var ids = lec_duty_id + ',';
					}

					var actionUrl = "/yisheng/webframe/index.php/Admin/LecDuty/delAjax";
						$.ajax({
						type: 'post',
								url: actionUrl,
								data: {ids:ids},
