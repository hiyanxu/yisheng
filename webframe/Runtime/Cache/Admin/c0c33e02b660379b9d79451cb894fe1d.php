...
<html>
	...
	<body>
		...
		$('#table').bootstrapTable({
					...
					method: 'get',
					url: "/yisheng/webframe/index.php/Admin/Workflow/ajaxIndex",
					//cache: false,
		...
		*/
		function add(){
			var index = layer.open({
						...
						content: ['/yisheng/webframe/index.php/Admin/Workflow/add', 'yes'],
						btn: ['确定', '取消']
						, yes: function (index) {
						var obj = layer.getChildFrame('#wt-forms', index); //获取form的值
								$.ajax({
									type: 'post',
									url: '/yisheng/webframe/index.php/Admin/Workflow/insert',
									data: obj.serialize(),
									cache: false,
		...
		*/
		function edit(workflow_id){
			var index = layer.open({
						...
						// time:1000,  设置自动关闭窗口时间 1秒=1000；
						shift: 1, //打开效果：0-6 。0放大，1从上到下，2下到上，3左到右放大，4翻滚效果；5渐变；6抖窗口
						content: ['/yisheng/webframe/index.php/Admin/Workflow/edit/workflow_id/'+workflow_id, 'yes'],
						btn: ['确定', '取消']
						, yes: function (index) {
						var obj = layer.getChildFrame('#wt-forms', index); //获取form的值
								$.ajax({
									type: 'post',
									url: '/yisheng/webframe/index.php/Admin/Workflow/update',
									data: obj.serialize(),
									cache: false,
									success: function (data) {
										if (data.status) {
		...
		*/
		function del(workflow_id){
			layer.confirm('确定要删除该工作流吗？', {
					...
					var actionUrl = "/yisheng/webframe/index.php/Admin/Workflow/del";
						$.ajax({
						type: 'post',
								url: actionUrl,
