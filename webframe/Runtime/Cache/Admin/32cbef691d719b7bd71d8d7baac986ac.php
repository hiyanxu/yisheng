...
<html>
	...
	<body>
		<div id="page-content">
			<div id="main-container container-fluid" style="margin-left:40px;">
				<div id="headshow" >
					<button type="button" class="btn btn-success btn-sm" onclick="add();"><span class="glyphicon glyphicon-plus"></span>工作流选取</button>
	...
	<script type="text/javascript">
		$('#table').bootstrapTable({
					...
					method: 'get',
					url: "/yisheng/webframe/index.php/Admin/LecWof/ajaxIndex",
					//cache: false,
			...
			function handle(value, row, index) {
				...
				var btnIsEnable='';
				if(row.isenable=="0"){
					btnIsEnable='&nbsp;&nbsp;<a class="remove ml10 btn btn-xs btn-outline btn-default" href="javascript:void(0)" onclick="setNewsWorkflow('+row.lec_workflow_id+',1)" title="设为禁用"><i class="glyphicon glyphicon-remove-circle"></i></a>';
				}
				else{
					btnIsEnable='&nbsp;&nbsp;<a class="remove ml10 btn btn-xs btn-outline btn-default" href="javascript:void(0)" onclick="setNewsWorkflow('+row.lec_workflow_id+',0)" title="设为启用"><i class="glyphicon glyphicon-off"></i></a>';
				}
				return [
						'<a class="remove ml10 btn btn-xs btn-outline btn-default" href="javascript:void(0)" onclick="delmore('+row.lec_workflow_id+')" title="删除">',
		...
		function responseHandler(res) {
			...
			function queryParams(params) {
				...
				if (typeof (params.sort) == "undefined") {
					params.sort = 'lec_workflow_ano.lec_workflow_id'; //默认排序字段
					params.order = 'desc';
				}
		...
		*/		
		function add(){
			var index = layer.open({
						type: 2,
						skin: 'demo-class',
						title: ['工作流选择', 'font-size:14px;color:black;'],
						...
						content: ['/yisheng/webframe/index.php/Admin/LecWof/add', 'no'],
						btn: ['确定', '取消']
						, yes: function (index) {
						...
						if(workflow_id==""){
							...
							return;
								...
								$.ajax({
									type: 'post',
									url: '/yisheng/webframe/index.php/Admin/LecWof/insert',
									data: obj.serialize(),
									cache: false,
		...
		*/
		function setNewsWorkflow(id,flag){
			$.ajax({
				...
				url:"/yisheng/webframe/index.php/Admin/LecWof/setLecWorkflowIsEnable",
				success:function(data){
					if (data.status) {
											layer.msg("设置成功", {
											icon: 1,
		...
		数据删除的方法  包括批量删除
		*/
		function delmore(lec_workflow_id){
			layer.confirm('确定要删除吗？', {
					...
					if (!lec_workflow_id) {
						...
						$.each(obj, function (n, value) {
							ids += value.lec_workflow_id + ',';
						});
					} else {
						var ids = lec_workflow_id + ',';
					}

					var actionUrl = "/yisheng/webframe/index.php/Admin/LecWof/delAjax";
						$.ajax({
						type: 'post',
								url: actionUrl,
								data: {ids:ids},
