...
*/
function getExAcesByWof($role=null,$wof_id=null,$wof_step_now=null,$ex_status=null){	
	...
	else{  //表示当前登录人不是admin，则我们根据当前登录角色和工作流步骤进行判断
		$is_need_role_row=M('workflowstep')->where("workflow_id={$wof_id} and role_id={$role}")->field("step_now")->first();  //当前工作流中是否有该角色
		$wof_ex_steps_row=M("workflow")->where("wof_id={$wof_id}")->field("steps")->first();  //获取该工作流的总步骤
